...
function getSmartphone(smartphone) {
    return `<tr><td>${smartphone.producer}</td><td>${smartphone.model}</td><td>${smartphone.price}</td>` +
        ...
        `<td><button onclick="updateSmartphone(${smartphone.id})">Edit</button></td></tr>`;
}
...
function updateSmartphone(id) {
    let producer = prompt("Nhập nhà sản xuất mới:");
    let model = prompt("Nhập model mới:");
    ...
    $.ajax({
        ...
        type: "PUT", // nên là PUT cho update
        url: `http://localhost:8080/api/smartphones/${id}`,
